...
def load_context_from_files(directory_path):
    ...
    # Adjust the pattern if files have different extensions
    for file_path in p.glob('HDFC BANK_*.txt'):
...
#     prediction, accuracy = csvfile.get_acc_pred(name)
...
# stock_ticker_name = model.generate_content(f"what is the stock ticker name for {comp_name} in yfinance. I need only the name. your response should only be the Stock ticker for eg if the company is Microsoft you should respond with : MSFT, if the company is HDFC then you should respond with : HDFCBANK.NS")
# print(stock_ticker_name.text)
...
def handle_query():
    ...
    prediction1,accuracy1=csvfile.get_acc_pred(stock_ticker_name)
    ...
    print(f"These are the news related to {stock_ticker_name}:{context} \n And here are the predictions for the stock for the next year{prediction1}\n And here are the accuray for the same:{accuracy1}")
...
def get_comp_name():
    ...
    data = request.json
    comp_name = data['query']  # Use 'query' to match React component's sent data
    stock_ticker_prompt = f"What is the stock ticker name for {comp_name} in yfinance?"
    
    # Simulate a model response or use a real model as needed
    ...
    # stock_ticker_name = "AAPL"  # Simulated response, replace with actual model response
    print(stock_ticker_name)
