...
def save_article_to_file(article, index, directory):
    """Save article content to a text file."""
    output_file_path = os.path.join(directory, f'sample_{index}.txt')
...
for i, article_data in enumerate(articles, start=1):
    ...
    if article_content:
        file_path = save_article_to_file(article_content, i, output_dir)
